---
name: ci-setup
description: Use when setting up or improving a CI/CD pipeline. Triggers: "set up CI", "GitHub Actions", "add CI/CD", "automate deployment", "pipeline". Generates a tailored GitHub Actions workflow with test, lint, build, and optional deploy gates.
---
...
- Deployment target (from AGENTS.md or user input)
...
- User-controlled input in `run:` blocks protected against injection via intermediate env vars

For complex infrastructure review, use a `generalPurpose` Task subagent to review the workflow for:
- Security misconfigurations
- Missing test gates before deploy
- Unpinned third-party actions (supply chain risk)
- Proper secret handling

## Phase 4: Document Required Secrets

List every secret the workflow needs in `.github/SECRETS.md` (or update AGENTS.md):
```
...
- An unpinned third-party action is a supply chain risk
