...
# Injects current git context, active plan, and session health into Cursor.
...
# Nag if AGENTS.md is still a placeholder
CONTEXT_FILE="$PROJECT_DIR/AGENTS.md"
if [ -f "$CONTEXT_FILE" ]; then
  if grep -q "project-setup" "$CONTEXT_FILE" 2>/dev/null; then
    echo ""
    echo "NOTE: AGENTS.md is still a placeholder. Run the project-setup skill to generate"
    echo "full project context (architecture, stack, conventions, verification commands)."
  fi
fi
...
" 2>/dev/null || echo "")
    if [ -n "$USED_PCT" ]; then
      ...
      echo "Consider using subagents or breaking the task into smaller pieces."
    fi
  fi
fi
